...
def dato_historico(moneda1='BTC', moneda2='USDT', timeframe='1m', desde=datetime.utcnow() - timedelta(weeks=24),
    ...
    logging.basicConfig(
        level=logging.INFO, format='{asctime} {levelname} ({threadName:11s}) {message}', style='{')
    ...
    while not finished:
        ...
        params = {'limit': limit, 'start': ultimaFecha,
                  'end': endTime, 'sort': '1'}
    ...
    df_acum = df_acum[['ticker', 'time', 'open',
...
def agregar_indicadores(df):
    ...
    # Agrego el valor forward de 10 minutos
    df['fw_60'] = (df.close.shift(-10) / df.close - 1)*100
...
df_forest.drop(['ticker', 'open', 'high', 'low',
